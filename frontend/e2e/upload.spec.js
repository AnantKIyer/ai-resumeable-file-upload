...
test.describe('File Upload E2E Tests', () => {
  ...
  test('should upload small file successfully', async ({ page }) => {
    ...
    // Wait for dropzone
    const dropzone = page.locator('[data-testid="dropzone"]').or(page.locator('input[type="file"]').locator('..'))
  ...
  test('should handle upload errors gracefully', async ({ page, context }) => {
    ...
    await context.route('**/api/upload/**', route => {
      if (route.request().method() === 'POST' && route.request().url().includes('/chunk')) {
        route.abort()
      } else {
        route.continue()
      }
  ...
  test('should support resume functionality', async ({ page }) => {
    ...
    // Reload page (simulating network failure)
    await page.reload()
    ...
    await expect(page.getByText(/resume|continue/i)).toBeVisible({ timeout: 10000 })
  })
  ...
  test('should display chunk status grid', async ({ page }) => {
    ...
    await expect(page.getByText(/chunk status/i)).toBeVisible({ timeout: 10000 })
  })
