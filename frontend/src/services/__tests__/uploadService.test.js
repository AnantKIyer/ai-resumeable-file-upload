...
describe("uploadService", () => {
  ...
  describe("uploadFile", () => {
    ...
    it("should handle upload errors", async () => {
      ...
      // Mock uploadChunk to always fail (will retry 3 times with delays)
      api.uploadChunk.mockRejectedValue(new Error("Network error"));

      // The test will timeout due to retries, so we increase the timeout
      // and expect it to throw after all retries fail
      await expect(
        uploadFile(file, {
          chunkSize: 1024 * 1024,
          concurrency: 1,
    ...
    }, 15000); // Increase timeout to 15 seconds to account for retries
